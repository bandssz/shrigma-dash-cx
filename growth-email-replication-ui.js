...
const GERU={
 ...
 open(id){
  if(GRU.state.ocupado||GRU.state.confirmando||GRU.emailTestSession||GRU.nativeEmailSession||GERU.session)return;
 ...
 html(){
  ...
  return `<section class="painel draft-confirm" id="email-replication" role="dialog" aria-modal="false" aria-labelledby="rep-title" tabindex="-1"><h3 id="rep-title">Copiar e-mail para ${e(b.name)}</h3><div class="rep-badges"><span class="rep-badge" title="A cópia fica neste dispositivo, sem publicação ou envio. A origem permanece intacta.">Novo rascunho local</span>${s.plan.links.some(l=>l.tracking_removed.length||l.directRequired)?'<span class="rep-badge" title="Identificadores do envio anterior e sua campanha UTM não acompanham a cópia. Revise a campanha de destino em cada endereço.">Rastreamento de envio removido</span>':''}</div>${s.error?`<p role="alert" class="drafts-msg" data-tone="erro">${e(s.error)}</p>`:''}
   ...
   ${s.review?`<div id="rep-final"><h4>Prévia da cópia · ${e(b.name)}</h4><dl class="rep-envelope"><dt>Remetente</dt><dd>${e(s.review.from_email)}</dd><dt>Responder para</dt><dd>${e(s.review.reply_to)}</dd><dt>Assunto</dt><dd>${e(s.review.assunto)}</dd><dt>Pré-header</dt><dd>${e(s.review.preheader)}</dd></dl><span class="rep-badge" title="Links e imagens externas ficam desativados nesta prévia. Confira os destinos listados abaixo.">Prévia protegida</span>${GMP.frame(s.nativePreview?.body_html||GEC.html(s.review),false,'Prévia da cópia para '+b.name)}<details><summary>Endereços finais revisados</summary><ul>${GER.urls(s.review).map(url=>`<li>${e(url)}<small class="rep-campaign">Campanha UTM: ${e(GER.campaigns(url).join(", ")||"não definida neste endereço")}</small></li>`).join('')||'<li>Sem endereços no conteúdo.</li>'}</ul></details><details><summary>HTML da cópia</summary><pre>${e(GEC.html(s.review))}</pre></details><label><input type="checkbox" id="rep-confirmed"${s.confirmed?' checked':''}>Revisei conteúdo, identidade visual e destinos para ${e(b.name)}.</label><div class="draft-confirm-row"><button type="button" class="btn" id="rep-create"${s.confirmed?'':' disabled'}>Criar rascunho de ${e(b.name)}</button></div></div>`:''}</section>`;
 },
 async review(){const s=GERU.session;if(!s||GRU.state.ocupado)return;try{GERU.current(s);const content=GER.apply(s.plan,{fields:s.fields,links:s.links});s.review=null;s.nativePreview=null;s.confirmed=false;
  if(typeof GENU!=='undefined'?GENU.advanced(content):GEC.templateExpressions(content.corpo,true).native){if(typeof GENU==='undefined')throw Error('A prévia nativa ainda não está disponível. Atualize o painel.');const input=JSON.stringify({fields:s.fields,links:s.links});GRU.state.ocupado='native_replication_preview';GRU.render();const preview=await GENU.previewDraft(content);GERU.current(s);if(GERU.session!==s||input!==JSON.stringify({fields:s.fields,links:s.links}))throw Error('A cópia mudou. Confira novamente.');s.nativePreview=preview;}
  s.review=content;s.error='';}catch(e){s.review=null;s.nativePreview=null;s.confirmed=false;s.error=e.message;}finally{GRU.state.ocupado=null;GRU.render();document.getElementById(s.review?'rep-confirmed':'rep-preview')?.focus();}},
 create(){
  const s=GERU.session;if(!s?.review||!s.confirmed||GRU.state.ocupado)return;
 ...
 cancel(){if(GRU.state.ocupado)return;const id=GERU.session?.plan.source_id;GERU.session=null;GRU.replicationSession=false;GRU.render();document.querySelector('[data-email-replicate="'+id+'"]')?.focus();},
 bind(root){
  root.querySelectorAll('[data-email-replicate]').forEach(b=>b.onclick=()=>GERU.open(b.dataset.emailReplicate));
  const box=root.querySelector('#email-replication'),s=GERU.session;if(!box||!s)return;if(GRU.state.ocupado){box.querySelectorAll('button,input').forEach(el=>el.disabled=true);return;}
