...
const GERU={
 ...
 html(){
  ...
  return `<section class="painel draft-confirm" id="email-replication" role="dialog" aria-modal="false" aria-labelledby="rep-title" tabindex="-1"><h3 id="rep-title">Copiar e-mail para ${e(b.name)}</h3><p>Será criado um novo rascunho neste dispositivo. Publicação, validações e tentativas de envio da origem não acompanham a cópia.</p>${s.error?`<p role="alert" class="drafts-msg" data-tone="erro">${e(s.error)}</p>`:''}
   ...
   <h4>Destinos e imagens</h4><p class="mini">Informe o endereço completo na outra marca. Produtos e páginas podem ter caminhos diferentes; confira-os antes de marcar como revisados. Esta etapa não consulta a loja nem confirma que os endereços existem. Recursos externos só serão mantidos com sua revisão.</p>
   ${s.plan.links.length?s.plan.links.map((l,i)=>`<div class="campo rep-link"><label for="rep-url-${i}">${l.kind==='origin'?'Endereço da marca de origem':l.kind==='relative'?'Endereço sem destino completo':l.kind==='dynamic'?'Endereço preenchido pela automação':'Recurso externo'} · ${e(l.url)}</label><input type="text" id="rep-url-${i}" data-rep-url="${i}" value="${e(s.links[i].target)}" placeholder="https://${e(b.domain)}/caminho-conferido"><label><input type="checkbox" data-rep-reviewed="${i}"${s.links[i].reviewed?' checked':''}>Conferi o destino e seu uso por ${e(b.name)}${l.kind==='dynamic'?' nos dados da automação':''}</label></div>`).join(''):'<p class="nota">Nenhum endereço encontrado. Confira também o texto e a identidade visual na prévia.</p>'}
   ...
   ${s.review?`<div id="rep-final"><h4>Prévia da cópia · ${e(b.name)}</h4><p class="mini">Remetente: ${e(s.review.from_email)} · Respostas: ${e(s.review.reply_to)}<br>Assunto: ${e(s.review.assunto)} · Pré-header: ${e(s.review.preheader)}<br>Links e imagens externas estão desativados na prévia. Os parâmetros de acompanhamento foram preservados e adaptados para a marca.</p>${GMP.frame(GEC.html(s.review),false,'Prévia da cópia para '+b.name)}<details><summary>Endereços finais revisados</summary><ul>${GER.urls(s.review).map(url=>`<li>${e(url)}</li>`).join('')||'<li>Sem endereços no conteúdo.</li>'}</ul></details><details><summary>HTML da cópia</summary><pre>${e(GEC.html(s.review))}</pre></details><label><input type="checkbox" id="rep-confirmed"${s.confirmed?' checked':''}>Revisei conteúdo, identidade visual e destinos para ${e(b.name)}.</label><div class="draft-confirm-row"><button type="button" class="btn" id="rep-create"${s.confirmed?'':' disabled'}>Criar rascunho de ${e(b.name)}</button></div></div>`:''}</section>`;
 },
