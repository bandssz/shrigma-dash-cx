...
const GA=(()=>{
 ...
 function render(ctx){
  ...
  const root=document.querySelector('#attribution-campaigns'),bar=document.querySelector('#attribution-status'),campaignStatus=document.querySelector('#attribution-campaign-status');if(!root||!bar)return;
  ...
  if(!ok){bar.innerHTML='<p>Atribuição por pedido ainda não disponível para este recorte. Valores da fonte anterior não foram reconciliados.</p>';if(campaignStatus){campaignStatus.hidden=false;campaignStatus.textContent='Atribuição indisponível neste recorte.';}root.innerHTML='<div class="vazio">Histórico indisponível nesta consulta. '+(typeof onRefresh==='function'?'<button type="button" class="refresh-btn" data-attribution-refresh>Atualizar dados</button>':'Atualize os dados e confira a marca e o período.')+'</div>';root.querySelector('[data-attribution-refresh]')?.addEventListener('click',()=>onRefresh());return;}
  ...
  if(campaignStatus){const warnings=[!cov.complete?'Total parcial: dias sem conciliação ficam fora dos resultados.':'',cov.pending?`${n(cov.pending)} pedido(s) com jornada pendente.`:'',cov.partial?`${n(cov.partial)} jornada(s) parcial(is); assistências podem estar incompletas.`:''].filter(Boolean);campaignStatus.hidden=warnings.length===0;campaignStatus.innerHTML=campaignStatus.hidden?'':`<span class="ga-coverage ${cov.complete?'is-complete':'is-partial'}" title="${e(warnings.join(' ')||'Cobertura de leitura não garante rastreamento completo.')}">${cov.complete?'Período conciliado':`Cobertura parcial · ${n(cov.covered)} de ${n(cov.expected)} dias × marca`}</span>${cov.pending||cov.partial?'<span class="mini"> · Há jornadas pendentes ou parciais</span>':''}`;}
