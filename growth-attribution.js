 ...
 function campaigns(api,b,a,z,channel='todos',search=''){
  ...
  for(const v of map.values()){
   v.members=v.members.map(m=>{
    ...
    return {...m,tracking_state:!tracked?'missing':[...tuples].some(k=>(plannedClaimants.get(k)?.size||0)>1)?'shared':'exclusive',in_period:sent&&inPeriod(day(m.enviado_em),a,z),future:!sent,shared,tracked,result:sent&&tracked&&exclusive.length?sum(uniqueMatches):null};
