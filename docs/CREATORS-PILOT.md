...
Grão do fato Meta: conta × anúncio × dia × modelo. A coleta solicita clique de sete dias, data de conversão, moeda BRL e fuso America/Sao_Paulo. Usa somente o campo explícito `7d_click` de `offsite_conversion.fb_pixel_purchase`; o campo genérico `value` não é fallback, pois pode divergir. Ausência do campo da janela permanece desconhecida. Não soma tipos `purchase`, `omni_purchase` e compra no site. A janela solicitada é declarada, sem inferir equivalência com qualquer configuração não verificada do Ads Manager.
...
Coletor diário às 06:17 de Brasília, janela dos sete dias completos anteriores. Contas são lidas em sequência; paginação tem limite e intervalo. Credencial fica no cofre. A base corrigida `crm_creator_meta_*_v2` foi criada separadamente: o conjunto inicial v1 fica preservado para auditoria e não alimenta esta leitura. A mudança de leitura exige conferir cada snapshot completo na base nova. A nova base só substitui a janela de uma conta depois de todas as páginas validadas; erro/paginação incompleta preservam a leitura anterior e marcam a fonte. Execução antiga não sobrescreve uma coleta mais nova. Os fatos históricos fora da janela ficam preservados, sem alegação de atualização contínua de todo o passado.
