...
| e | `crm_wa_template` não declara cobertura. Sem ela, um array vazio (ou sem linha para um template no período) não prova zero — e a revisão de 11/09 (F01) apontou que o front convertia isso em 0. | Top-level opcional `"crm_wa_template_cobertura":{"inicio":"2026-06-10","fim":"2026-09-07"}`, mesma semântica de `crm_wa_cobertura`, mais a declaração de **grão e população** da coleção: uma linha por `(dia, marca, flow, piece, template_ref)`, sem linha-agregado da peça no mesmo array (B06), e se `flow:teste-motor` entra (o front exclui como em Envios; se o backend já excluir, dizer). | Front mostra "Sem linha no período · cobertura não declarada", célula vazia no CSV e etiqueta "cobertura não declarada" nas somas; soma por campo vira "—" se qualquer linha tiver contagem não informada. |
| f | `crm_fontes.status` só tinha `ok`/`atrasado`/`error` nos exemplos; a revisão (F02/C01) pede tabela fechada. | O front trata `status ∈ {ok, atrasado, error|erro}` e `tipo:evento`; qualquer outro valor (ou ausência com hora) vira "status não informado" e nunca herda ok. Pedido: fixar esse conjunto no contrato, `cadencia_seg:null` para push, e informar `erro` quando `status:error`. O front não aplica limiar próprio ("2× a cadência" foi removido): atraso é o que a API marcar. | Como acima — "desconhecido" é visível na faixa. |
