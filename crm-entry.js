...
(function(){
 ...
 function logout(text='Acesso encerrado neste navegador.'){
  epoch++;key='';identity=null;selected='';clearTimeout(expiry);expiry=null;frame?.remove();frame=null;nav.replaceChildren();nav.hidden=true;shell.hidden=true;login.hidden=false;field.value='';controls.disabled=false;cancel.hidden=true;busy=false;message.textContent=text;clearLegacy();field.focus();
 }
 ...
 function enter(i,k){
  clearLegacy();identity=i;key=k;field.value='';login.hidden=true;shell.hidden=false;message.textContent='';
  document.getElementById('entry-owner').textContent=i.role==='master'?'Dono do projeto':i.owner||'Gestor da área';
