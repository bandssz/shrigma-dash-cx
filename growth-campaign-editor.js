...
const GCE=(()=>{
 ...
 function sameContext(before,{allowRecovery=false}={}){
 ...
 function bindRemote(){
  ...
  q('[data-ce-recover]').addEventListener('click',()=>{
   ...
   confirmAction(`Recuperar ${brand} · “${c.definition.name}” (campanha ${c.id})? O servidor confirmou um rascunho com 0 envios, ainda não iniciado. Seu conteúdo original será preservado para correção. Esta ação não cria outra campanha, não agenda e não envia.`, 'Recuperar este rascunho',confirmed=>runRemote(()=>client.recover(proof,'recuperar'),{fillSaved:true,write:true,confirmed,recoverLocal:true}),{allowRecovery:true});
  });
