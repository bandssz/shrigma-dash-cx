...
const GFU={
  ...
  render(ctx){
    ...
    const contexto=`<div class="control-context"><div><strong>Fluxos · leitura</strong><span class="control-badge" title="A edição de fluxos não está disponível nesta consulta. Nenhuma configuração é alterada aqui.">Somente leitura</span><p>${lista.presenteDef?`Configuração consultada em ${GFU.e(lista.gerado_em?GFU.stamp(lista.gerado_em):'horário não informado')}.${lista.observados.length?' Fluxos sem configuração informada aparecem como "observado no motor".':''}`:'<strong>Gatilho, ordem e esperas não informados.</strong> As mensagens observadas aparecem abaixo; confira os serviços em Operação atual.'}</p>
      <details class="control-detail" data-gt-key="fluxos-como-ler"><summary>Como ler estes fluxos</summary><p>${lista.presenteDef?'Gatilho, etapas, esperas e condições seguem a configuração recebida.':'A consulta informa peças, canais, templates, serviços e modo. O nome de uma peça não revela o gatilho nem a ordem de execução.'} Modo é a configuração do serviço; "real" não confirma entrega. Volumes usam o período selecionado: WhatsApp aceitos e entregues; e-mail aceito pelo provedor.</p></details></div>
