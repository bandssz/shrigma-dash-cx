...
function pintaConcessaoAba(d) {
  ...
  const cartoes = [
    { k: "pct", rot: `Concessão · ${fmtMes(mesAnt)}`, val: pctTxt(fechado), status: cxStatus("concessao_pct", fechado.pct), chip: cxChipPP("co_pct", fechado.pct, fechadoAnt.pct, 2),
      ...
      info: `Reembolsos do ClickUp que o financeiro já executou (status feito, redigindo resposta, retorno concluído) ÷ receita paga da Shopify do último mês fechado (${fmtMes(mesAnt)}). ${CX_ALVOS.concessao_pct.rot} — meta do Head de CX ainda por fixar. Chip contra ${fmtMes(mesAnt2)}.` + porMarca((m) => pctTxt(ag(mesAnt, mesAnt, [m]))) },
  ...
  if (rot) rot.innerHTML = (leitura ? cxTag(`leitura de ${fmtDia(leitura)}`, "nota", "Última leitura do ClickUp (01:50) e da receita Shopify (01:40).") : "") +
    ...
    cxTag("receita = pedidos pagos", "nota", "Receita é a soma dos pedidos pagos e não cancelados criados no mês (Shopify, status pago / parcialmente estornado / estornado), igual nas duas marcas. Não é o 'total de vendas' do Analytics, que soma também PIX expirado e pedido cancelado (+11% na Aris e +14% na Fish em ago/26).") +
    cxResumoStatus(cartoes);
