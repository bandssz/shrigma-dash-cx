...
const {patchWorkflow}=require('../n8n/growth/campaign-runtime-patch.cjs');
const load=()=>{const f=JSON.parse(fs.readFileSync(path.join(__dirname,'fixtures/campaign-audience-runtime-before.json')));for(const n of f.workflow.nodes)if(n.parameters.jsCode)n.parameters.jsCode=n.parameters.jsCode.replace('__PREVIOUS_BUNDLE__',()=>f.previousBundle);return f.workflow;};
const get=(w,n)=>w.nodes.find(x=>x.name===n);
test('audience patch changes exactly five code bodies and preserves live auth, CORS and credentials',()=>{
 ...
 const c=get(w,'Entrada').parameters.jsCode;assert.ok(c.includes("'audience_review_id'"));assert.ok(c.includes('req.headers?.authorization'));assert.ok(c.includes("authHeader.slice(7)"));
 assert.match(get(w,'Despacha').parameters.jsCode,/"cancel","review"/);assert.match(get(w,'Recibo erro PG').parameters.jsCode,/AUDIENCE_CHANGED/);
