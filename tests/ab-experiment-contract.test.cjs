...
function evidence(){return {contract:AB.CONTRACT,test_id:config().test_id,protocol:config(),integrity:{allocation_complete:true,assignment_disjoint:true,transport_bound:true,transport_continuous:true,tracking_continuous:true,source_complete:true},arms:[{arm:'a',campaign_id:1,allocated:10000,unique_clickers:100,native_sent:10000,revoked:10,unknown:0,finished_before_deadline:true},{arm:'b',campaign_id:2,allocated:10000,unique_clickers:2000,native_sent:10000,revoked:200,unknown:0,finished_before_deadline:true}],window_start:'2026-09-25T12:00:00Z',window_end:'2026-09-26T12:00:00Z',as_of:'2026-09-26T12:00:00Z'};}
...
test('fixed mature evidence can declare a rule-based winner, with no automatic sends',()=>{
 ...
 // Revocations can overlap people already sent. They are never added to sent.
 assert.equal(r.arms[1].allocated,10000);
...
test('missing/anonymous/cross-arm/deleted/out-of-range data never become zero or a winner',()=>{
 for(const mutate of [s=>s.integrity.transport_bound=false,s=>s.integrity.transport_continuous=false,s=>s.integrity.assignment_disjoint=false,s=>s.integrity.tracking_continuous=false,s=>s.integrity.source_complete=false,s=>s.arms[1].unknown=1,s=>s.arms[1].allocated=0,s=>s.arms[1].unique_clickers=null,s=>s.arms[0].native_sent=10001,s=>s.arms[1].native_sent=0,s=>s.window_end='2026-09-27T12:00:00Z',s=>s.protocol.rule.minimum_effect_pp=99]){
...
test('finished but unaccounted transport is inconclusive, even if revoked numerically fills the deficit',()=>{
 for(const [sent,revoked,clicks] of [[0,0,0],[9999,1,2000],[9800,200,2000]]){
  const s=evidence();Object.assign(s.arms[1],{native_sent:sent,revoked,unique_clickers:clicks});const r=AB.result(config(),s);
  assert.equal(r.status,'inconclusive');assert.equal(r.reason,'transport_not_fully_accounted');assert.equal(r.winner,null);assert.equal(r.can_declare_winner,false);assert.equal(r.automatic_send,false);
  assert.equal(r.arms[1].allocated,10000);assert.equal(r.arms[1].native_sent,sent);assert.equal(r.arms[1].revoked,revoked);assert.equal(r.arms[1].rate,clicks/10000);
 }
});
