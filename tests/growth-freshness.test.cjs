...
function fixture(){return {crm_diario:[{marca:'fish',coletado_em:stamp(0)}],crm_intradia:[{marca:'olivas',coletado_em:stamp(27*60)}],crm_conversao:[],crm_collection_receipt:[{source:'legacy_conversion',brand:'olivas',checked_at:stamp(5)}],crm_attribution:{schema_version:2,coverage:['fish','aristo','olivas'].map(brand=>({brand,day:'2026-09-16',checked_at:stamp(5)})),hourly:[{marca:'fish',dia:'2026-09-16',model:'last_click',receita:100}]}};}
...
test('brand selection isolates alerts; all brands still includes a real Olivas delay',()=>{const f=fixture();f.crm_attribution.coverage.find(x=>x.brand==='olivas').checked_at=stamp(28*60);assert.equal(badge(F.sources(f,'fish')).red,false);assert.match(badge(F.sources(f,'todas')).textContent,/Olivas do Campo sem atualização/);});
...
test('a legacy sale timestamp alone is not proof of a successful source read',()=>{const f=fixture();f.crm_attribution.coverage=f.crm_attribution.coverage.filter(x=>x.brand!=='olivas');assert.match(badge(F.sources(f,'olivas')).textContent,/sem confirmação/);});
