...
test('initial load and period change query only Creators; opening TikTok reads the current range',()=>{
 ...
 p.select('afil');assert.deepEqual(p.reads.at(-1),{area:'tiktok',ini:'2026-08-01',fim:'2026-08-31'});
...
test('replacing read access refreshes only the visible domain and the shared health strip',async()=>{
 ...
 p.select('afil');p.reads.length=0;await p.login();assert.deepEqual(p.reads.map(x=>x.area),['tiktok','health']);
});
test('invalid dates do not query; rapid tab switching uses the newest chosen period',()=>{
 ...
 p.range('2026-07-01','2026-07-31');p.select('afil');p.range('2026-08-01','2026-08-31');p.select('creators');
 assert.deepEqual(p.reads.map(x=>[x.area,x.ini]),[['creators','2026-07-01'],['tiktok','2026-07-01'],['tiktok','2026-08-01'],['creators','2026-08-01']]);
});
test('an unavailable hidden TikTok loader cannot prevent Creators period selection',()=>{
 ...
 assert.deepEqual(p.reads.map(x=>x.area),['creators']);p.select('afil');assert.equal(p.reads.length,1);
});
