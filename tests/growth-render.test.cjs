...
for(const brand of ['fish','aristo'])test('replication wizard '+brand+' creates only a reviewed local draft for the other brand',async()=>{
 ...
 assert.equal(x.document.querySelector('#drafts-novo-email').disabled,true);assert.equal(x.document.querySelector('#rep-create'),null);
 ...
 assert.equal(x.document.querySelector('#email-replication'),null);const rows=JSON.parse(x.run('JSON.stringify(GR.lista())'));assert.equal(rows.length,2);const copy=rows.find(r=>r.id!==id);assert.equal(copy.marca,to);assert.equal(copy.servidor,undefined);assert.ok(copy.corpo.includes('utm_source='+to));assert.equal(x.calls.length,beforeCalls);assert.equal(x.run('GRU.contextStatus().blocked'),false);
});
