...
test('brand changes select isolated CRM tokens while other panels and status semantics stay untouched',()=>{
 ...
 assert.equal(fish['--crm-sidebar-bg'],'#f8f9fa');assert.equal(aristo['--crm-sidebar-bg'],'#fffbec');
 ...
 for(const theme of [fish,aristo,all,olivas]){
  assert.equal(theme['--fundo'],'#fff');assert.equal(theme['--texto'],'#202d3a');
...
test('brand text, selection, primary action and focus colors retain readable contrast',()=>{
 for(const brand of ['fish','aristo','todas']){
  ...
  for(const bg of [sidebar,t['--crm-accent-bg'],t['--crm-hover-bg'],'#fff'])assert.ok(contrast(accent,bg)>=4.5,`${brand}: accent on ${bg}`);
  for(const fg of [t['--texto'],t['--mudo'],t['--rotulo']])for(const bg of [sidebar,t['--crm-hover-bg'],'#fff'])assert.ok(contrast(fg,bg)>=4.5,`${brand}: ${fg} on ${bg}`);
