...
const GB={
 ...
 async request(action,body={},write=false){
  ...
  let k=localStorage.getItem(write?GTA.CHAVE_ESCRITA:GTA.CHAVE_LEITURA)||'';
 ...
 render(ctx){
  ...
  const rail=`<aside class="builder-rail"><div class="builder-rail-title">Jornadas <span>${visible.length}</span></div>${visible.map(x=>`<button type="button" class="builder-flow ${x.key===s.selected?'selected':''}" data-flow-key="${e(x.key)}"><span class="builder-dot ${x.enabled?'on':''}"></span><span><strong>${e(x.name)}</strong><small>${x.brand==='fish'?'Fishermans':'O Aristocrata'} · ${x.draft.steps.length} etapas</small></span></button>`).join('')}${!visible.length?'<p>Nenhum fluxo carregado.</p>':''}<button type="button" class="builder-reload" id="builder-reload" ${s.busy?'disabled':''}>↻ Atualizar</button></aside>`;
  const available=f?f.available_steps.filter(x=>!d?.steps.some(y=>y.key===x.key)):[];
  const main=f&&d?`<main class="builder-main"><header class="builder-header"><div><span class="builder-eyebrow">${f.brand==='fish'?'FISHERMANS':'O ARISTOCRATA'} / AUTOMAÇÃO</span><input class="builder-title" aria-label="Nome do fluxo" id="builder-name" value="${e(d.name)}" maxlength="120"><div class="builder-status"><span class="builder-status-pill ${f.enabled?'live':''}">${!f.runtime_ready?'Em preparação':f.enabled?'Ativo':'Pausado'}</span><span>Publicada v${f.published_version}</span>${s.dirty||f.version!==f.published_version?'<span class="builder-draft-label">Alterações em rascunho</span>':''}</div></div><div class="builder-header-actions"><button type="button" id="builder-toggle" ${s.busy||s.dirty||!f.runtime_ready?'disabled':''}>${f.enabled?'Pausar':'Retomar'}</button><button type="button" id="builder-save" ${s.busy||!s.dirty?'disabled':''}>Salvar rascunho</button><button type="button" class="builder-primary" id="builder-publish" ${s.busy||s.dirty||!f.runtime_ready?'disabled':''}>Publicar alterações</button></div></header>
   <div class="builder-canvas"><div class="builder-trigger"><span class="builder-trigger-icon">↯</span><div><small>ENTRADA NO FLUXO</small><strong>${e(f.trigger)}</strong><p>${e(f.binding_description||'Cada evento mantém sua identidade para evitar envios repetidos.')}</p></div></div>
   ${GB.stagesHtml(f,d)}
  ...
  root.innerHTML=`${s.error?`<div class="builder-alert" role="alert">${e(s.error)}</div>`:''}${s.notice?`<div class="builder-notice" role="status">${e(s.notice)}</div>`:''}<div class="builder-layout" aria-busy="${s.busy}">${rail}${main}</div>`;GB.bind(root);
 },
 bind(root){
