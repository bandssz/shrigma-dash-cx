/* Durable browser fence for the legacy A/B registry. No server idempotency or
   experimental assignment is implied. Readback never calls a mutation. */
(function(root,factory){'use strict';const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.GABJ=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function create({endpoint,storage,locks,match,receipt,now=()=>Date.now(),uuid=()=>crypto.randomUUID()}={}){
  ...
  let memory=null;
  const available=()=>!!(normalized&&storage?.getItem&&storage?.setItem&&locks?.request&&typeof match==='function'&&typeof receipt==='function');
  function read(){
  ...
  async function run(input,{api,readProof}={},transport){
   const expected=normalize(input);if(typeof transport!=='function')throw fail('AB_INPUT','Transporte ausente.');
   return exclusive(async()=>{
    ...
    const op={id:uuid(),endpoint:normalized,expected,phase:'pending',startedAt:now()};j.operations.push(op);persist(j); // Durable before the only transport.
    let response;try{response=await transport(copy(expected));}catch{op.phase='uncertain';try{persist(j);}catch{}return {phase:'uncertain',id:op.id};}
  ...
  return Object.freeze({inspect,reconcile,run});
 }
