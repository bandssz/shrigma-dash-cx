...
const MESSAGES={AB_V2_CAMPAIGN_FROZEN:'Esta campanha pertence a um experimento A/B. Use o controle do experimento; seu conteúdo não foi alterado.',AB_V2_SCHEDULE_REQUIRED:'As variantes A/B devem ser agendadas ou canceladas juntas no controle do experimento.',RECOVERY_UNAVAILABLE:'Este rascunho não pode ser recuperado desta tentativa.',RECOVERY_ALREADY_CLAIMED:'Outra tentativa já recuperou este rascunho. Consulte a recuperação existente.',RECOVERY_INVALID:'A identidade da recuperação não foi confirmada.',AUDIENCE_REVIEW_REQUIRED:'Valide o público da versão salva antes de agendar.',AUDIENCE_STALE:'A conferência do público venceu. Valide novamente antes de confirmar.',AUDIENCE_CHANGED:'O público mudou desde a conferência. Valide e confira novamente.',AUDIENCE_EMPTY:'Não há destinatários elegíveis agora.',AUDIENCE_DISABLED:'Há destinatários desativados no público nativo. Resolva a política antes de agendar.',CAMPAIGN_EDITOR_REQUIRED:'Esta campanha é gerida pelo painel. Recarregue a versão atual antes de editar.',CAMPAIGN_REVIEW_REQUIRED:'A campanha precisa ser validada e agendada pelo painel.',CAMPAIGN_DEPENDENCY_IN_USE:'Uma campanha agendada, em execução ou pausada utiliza este recurso. Cancele a campanha antes de alterar o recurso.'};
const ERRORS=new Set(['AB_V2_CAMPAIGN_FROZEN','AB_V2_SCHEDULE_REQUIRED','RECOVERY_UNAVAILABLE','RECOVERY_ALREADY_CLAIMED','RECOVERY_INVALID','AUDIENCE_REVIEW_REQUIRED','AUDIENCE_STALE','AUDIENCE_CHANGED','AUDIENCE_EMPTY','AUDIENCE_DISABLED','CAMPAIGN_RECEIPT_MISMATCH','CAMPAIGN_OPERATION_INVALID','CAMPAIGN_NOT_FOUND','CAMPAIGN_SCOPE','VERSION_CONFLICT','CAMPAIGN_LOCKED','LIST_SCOPE','TEMPLATE_SCOPE','TEMPLATE_CHANGED','INITIATIVE_INVALID','INITIATIVE_CONFLICT','VALIDATION_STALE','SCHEDULE_TOO_SOON','INITIATIVE_MISSING','CONTENT_UNVALIDATED','CONTENT_EMPTY','CAMPAIGN_EDITOR_REQUIRED','CAMPAIGN_REVIEW_REQUIRED','CAMPAIGN_DEPENDENCY_IN_USE','CAMPAIGN_CREATE_DRAFT_ONLY','CAMPAIGN_ADOPTION_REQUIRED']);
function createProvider({query,nativeCreate,validateContent}){
 ...
 return {
  recovery:({sourceOperationId,actor})=>call('recovery_inspect',{sourceOperationId,actor}),
  recover:(id,{expectedVersion,operationId,sourceOperationId})=>call('recover',{id,expectedVersion,operationId,sourceOperationId}),
