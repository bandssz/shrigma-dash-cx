...
DO $ledger_shape$
DECLARE columns_shape jsonb; constraints_shape jsonb; expected_constraints jsonb := $constraints$[{"contype":"c","def":"CHECK (((state <> 'completed'::text) OR ((finished_at IS NOT NULL) AND (response IS NOT NULL))))"},{"contype":"c","def":"CHECK ((action = ANY (ARRAY['criar'::text, 'encerrar'::text])))"},{"contype":"c","def":"CHECK ((actor_sha256 ~ '^[a-f0-9]{64}$'::text))"},{"contype":"c","def":"CHECK ((state = ANY (ARRAY['running'::text, 'completed'::text])))"},{"contype":"p","def":"PRIMARY KEY (operation_id)"}]$constraints$::jsonb;
BEGIN
 ...
 -- Constraint order is not semantic and varies with the database collation.
 -- Match the exact set and cardinality so missing, changed or duplicate checks still fail.
 IF jsonb_array_length(constraints_shape) IS DISTINCT FROM jsonb_array_length(expected_constraints)
 OR (constraints_shape @> expected_constraints AND expected_constraints @> constraints_shape) IS DISTINCT FROM true
