...
`ab-operator.sql` adiciona uma função Growth que consulta `shrigma_crm_operator_auth_v1`, sem alterar login, cache ou a função compartilhada. `read_content` permite capacidades/registro/recibo; `draft` permite gravar o registro descritivo de Fish e Aristo. A autoria deriva do rótulo autenticado, com domínio próprio, e continua igual após rotação da chave do mesmo operador. O chamador não escolhe o ator.
