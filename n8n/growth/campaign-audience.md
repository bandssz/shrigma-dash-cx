...
A tela deve mostrar elegíveis, descartes, desativados, horário da conferência e validade antes de confirmar. A confirmação deve capturar a mesma marca, revisão e versão da campanha; edição, expiração, mudança de marca ou conflito exige nova validação. Conteúdo local sem revisão remota não pode ficar agendável. A consulta de recibos antigos não deve depender do novo campo.

A integração combinada deverá anunciar `campaigns.audience_review:'listmonk-6.1-regular-v1'` nas capacidades e exigir esse valor para novos agendamentos. Este commit de backend não publica essa capacidade nem modifica o cliente existente. Não anunciar suporte antes de concluir backend, cliente, testes e conferências do ambiente.
...
No ponto de integração somente backend, sete testes da tela/cliente anterior falham por não terem a revisão nova. São incompatibilidades reais a resolver junto à UI, sem relaxar o contrato. A publicação final exige essa regressão verde, a prova concorrente em CI e a verificação da implantação. Nenhum teste aqui comprova entrega de e-mail.
