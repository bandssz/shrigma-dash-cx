...
const CampaignTracking=(()=>{
 ...
 function prepare(c,{brand,campaign,now=Date.now()}={}){
  ...
  if(!id(c.id)||!['draft','scheduled'].includes(c.status)||Number(c.sent)!==0||c.started_at)throw Error('Only unsent draft/scheduled campaigns can be prepared');
  ...
  return {id:c.id,token,lists,body,altbody,changes};
 }
 return {prepare};
})();
