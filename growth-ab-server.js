...
(function(root,factory){const api=factory(typeof module==='object'&&module.exports?require('./growth-ab-protocol.js'):root.GABProtocol);if(typeof module==='object'&&module.exports)module.exports=api;else root.GABServer=api;})(typeof globalThis!=='undefined'?globalThis:this,function(Protocol){
 ...
 function validServer(op){try{return op?.server?.contract==='ab_registry_v1'&&hash(op.server.actor_sha256)&&same(op.server.payload,canonicalRequest(op.expected,op.server.payload?.expected_version))&&!!Protocol.request({operation_id:op.id,...op.server.payload},op.server.actor_sha256);}catch{return false;}}
 ...
 function create({endpoint,key,fetch:request=globalThis.fetch}={}){
  ...
  if(typeof key!=='string'||!key||typeof request!=='function')throw fail('Informe a chave de escrita para consultar a mesma operação.');
  async function call(query,body){
   ...
   if(!body&&(r.status===401||r.status===403))throw Object.assign(fail('A chave de escrita foi recusada. Informe o acesso da mesma tentativa e consulte novamente; nenhum POST será repetido.'),{code:'AB_AUTH_REQUIRED'});
  ...
  return {
   async prepare(expected,id,{api}={}){
    ...
    return {contract:'ab_registry_v1',actor_sha256:op.actor_sha256,payload};
   },
